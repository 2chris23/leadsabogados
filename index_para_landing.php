...
// Si este archivo está en httpdocs, la ruta al CRM en Plesk probablemente sea:
// /var/www/vhosts/leadsabogados.com/app.leadsabogados.com/portal/crm
$posible_ruta_plesk = '/var/www/vhosts/leadsabogados.com/app.leadsabogados.com/portal/crm';

// Si existe la ruta absoluta de Plesk, la usamos. Si no, probamos con rutas relativas locales para XAMPP
if (file_exists($posible_ruta_plesk . '/includes/config.php')) {
    define('CRM_ROOT', $posible_ruta_plesk);
} elseif (file_exists(__DIR__ . '/../app.leadsabogados.com/portal/crm/includes/config.php')) {
    define('CRM_ROOT', __DIR__ . '/../app.leadsabogados.com/portal/crm');
} else {
    // Fallback original
...
<head>
    ...
    <style>
        *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .cf-section-title { font-size: .875rem; font-weight: 800; color: var(--primary); text-transform: uppercase; letter-spacing: 0.05em; text-align: center; margin: 32px 0 24px; position: relative; }
        .cf-section-title::before, .cf-section-title::after { content: ''; position: absolute; top: 50%; width: calc(50% - 100px); height: 1px; background: #e2e8f0; }
        .cf-section-title::before { left: 0; }
        .cf-section-title::after { right: 0; }
    </style>
</head>
<body>
            ...
            <div class="consult-card">
                ...
                <form method="POST" action="#consulta" id="formConsulta" autocomplete="off" enctype="multipart/form-data">
                    ...
                    <div class="cf-row">
                        ...
                        <div class="cf-group">
                            <!-- Espacio vacío o para otro campo si hace falta -->
                        </div>
                    </div>
                    ...
                    <div class="cf-group">
                        ...
                        <div class="cf-radio-grid">
                            ...
                            foreach($tipos as $t): 
                                $checked = ($formData['tipo_problema'] === $t) ? 'checked' : '';
                            ?>
                            <div class="cf-radio-item">
                                <input type="radio" name="tipo_problema" id="tp_<?php echo md5($t); ?>" value="<?php echo esc($t); ?>" <?php echo $checked; ?> required>
                                <label for="tp_<?php echo md5($t); ?>"><?php echo esc($t); ?></label>
                            </div>
                            <?php endforeach; ?>
                        </div>
                    </div>
